...
router.get("/edit",isLoggedin, asyncWrap(async (req, res) => {
  ...
  console.log(data);
  const user = await User.find({});
  res.render("./MyStudentFamily/show", { data, cloud_name, facebook: user[0].facebook,
    twitter: user[0].twitter,
    linkedin: user[0].linkedin, });
}));
...
router.get("/:id/edit",isLoggedin, asyncWrap(async (req, res) => {
  ...
  console.log(data);
  const user = await User.find({});
  res.render("./MyStudentFamily/edit", { data: data[0], cloud_name, facebook: user[0].facebook,
    twitter: user[0].twitter,
    linkedin: user[0].linkedin,});
}));
